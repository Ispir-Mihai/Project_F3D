...
public class F3DCharacterController : MonoBehaviour
{
    ...
    public F3DGameManager F3DGameManager;
    ...
    private void Awake()
    {
        _fallDetector = GameObject.FindGameObjectWithTag("FallDetector").transform;
    ...
    private void Update()
    {
        ...
        _fallDetector.position = new Vector3(transform.position.x, _fallDetector.position.y);
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "FallDetector")
            F3DGameManager.RestartGame();

        else if (collision.gameObject.tag == "Enemy")
        {
            Destroy(Instantiate(bloodPrefab, collision.contacts[0].point, Quaternion.identity), 1.5f);
            _character.OnDamage(20);
        }
    }
